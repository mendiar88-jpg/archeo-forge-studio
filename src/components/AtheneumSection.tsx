...
const articles = [
  {
    title: "Information Governance in the AI Era",
    tag: "Governance",
    icon: Shield,
    content:
    ...
    takeaway: "AI governance isn't optional — it's the next evolution of records management.",
  },
  {
    title: "The Aesthetics of Order",
    tag: "Philosophy",
    icon: Eye,
    content:
      "There is an inherent beauty in well-structured systems. From the precise cataloging methods of archival science to the clean architecture of a well-designed database schema, order is not merely functional — it is aesthetic. The INTJ mind recognizes that elegant solutions are often the most efficient ones. When a workflow is designed with the same intentionality as a piece of architecture, every element serves a purpose, nothing is superfluous, and the result is both operationally sound and intellectually satisfying.",
    takeaway: "True efficiency is indistinguishable from elegance.",
  },
  {
    title: "Kaizen in Digital Operations",
    tag: "Process",
    icon: Zap,
    content:
      "The Japanese philosophy of Kaizen — continuous improvement through small, incremental changes — is profoundly applicable to modern digital operations. Rather than waiting for massive system overhauls, operational excellence comes from daily micro-optimizations: refining a single workflow step, automating one repetitive task, improving one metric by 1%. In BPO environments handling thousands of daily interactions, these marginal gains compound into transformative operational improvements over quarters and years.",
    takeaway: "1% better every day compounds into operational transformation.",
  },
  {
  ...
  },
  {
    ...
    takeaway: "The best compliance is the kind you never have to think about.",
  },
  {
    title: "The Terminal Mindset",
    tag: "Philosophy",
    icon: BookOpen,
    content:
      "In a world of polished GUIs and consumer-friendly interfaces, there's power in thinking like a terminal operator. The terminal mindset values precision over aesthetics, commands over clicks, and scriptable workflows over manual processes. It's the understanding that behind every beautiful dashboard is a query, behind every automated report is a cron job, and behind every 'seamless' user experience is someone who understood the system deeply enough to make complexity invisible.",
    takeaway: "Mastery means understanding the layer beneath the interface.",
  },
];
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {articles.map((a, i) => {
          ...
          return (
            <article
              key={i}
              className="group glass-card p-6 rounded-sm flex flex-col"
            >
              ...
              <h3 className="font-serif text-lg text-bone mb-3 group-hover:neon-text-gold transition-colors duration-300">
                {a.title}
              </h3>

              <p className="text-xs text-bone-dim leading-relaxed mb-4 flex-1">
                {a.content}
              </p>

              <div className="border-t border-border pt-3 mt-auto">
                <p className="font-mono text-[10px] tracking-wider uppercase neon-text-cyan">
                  ▸ {a.takeaway}
                </p>
              </div>
            </article>
